...
def preprocess_document(raw_text: str, filepath: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        if not header_done:
            ...
            if clean_line.startswith("Source:"):
                ...
                # Gặp section heading đầu tiên -> kết thúc header metadata
                header_done = True
                content_lines.append(line)
            # Bỏ qua các dòng trống hoặc dòng Title (chữ in hoa) ở header
            elif not clean_line or clean_line.isupper():
                continue
            else:
                # Nếu gặp dòng text bình thường mà chưa có ===, coi như đã hết header
                header_done = True
                content_lines.append(line)
        else:
            content_lines.append(line)
    ...
    cleaned_text = re.sub(r"\n{3,}", "\n\n", cleaned_text) # Chuẩn hóa khoảng trắng
...
def chunk_document(doc: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Chunk tài liệu theo section heading, sau đó split tiếp nếu cần.
    """
    text = doc["text"]
    base_metadata = doc["metadata"].copy()
    chunks = []

    # Split theo pattern "=== Section ... ===" hoặc "=== Phần ... ==="
    # Dùng regex capturing group để giữ lại cả heading
    ...
    for part in parts:
        if re.match(r"===.*?===", part):
            # Lưu section cũ trước khi sang section mới
...
def _split_by_size(
    ...
    """
    Helper: Tách nội dung section thành các chunk nhỏ hơn dựa trên paragraph.
    """
    ...
    for p in paragraphs:
        # Nếu paragraph quá dài đơn độc, phải cắt theo ký tự (hiếm gặp trong tập dữ liệu này)
        if len(p) > chunk_chars:
            ...
            # Cắt p ra thành các đoạn nhỏ theo characters
            p_start = 0
            while p_start < len(p):
                p_end = min(p_start + chunk_chars, len(p))
                chunks.append({
                    "text": p[p_start:p_end],
                    "metadata": {**base_metadata, "section": section},
                })
                p_start = p_end - overlap_chars
        
        # Thử ghép paragraph vào chunk hiện tại
        elif len(current_chunk_text) + len(p) + 2 <= chunk_chars:
            if current_chunk_text:
                current_chunk_text += "\n\n" + p
            else:
                current_chunk_text = p
        else:
            # Chunk hiện tại đã đầy, lưu lại và bắt đầu chunk mới
            chunks.append({
                "text": current_chunk_text,
                "metadata": {**base_metadata, "section": section},
            })
            # Lấy đoạn cuối của chunk cũ làm overlap (đơn giản hóa bằng paragraph cuối)
            current_chunk_text = p
...
def get_embedding(text: str) -> List[float]:
    ...
    # Model hỗ trợ đa ngôn ngữ phù hợp với dữ liệu tiếng Việt/Anh trộn lẫn
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    for filepath in doc_files:
        ...
        # 1. Preprocess
        doc_obj = preprocess_document(raw_text, str(filepath.name))

        # 2. Chunk
        chunks = chunk_document(doc_obj)

        # 3. Embed và lưu từng chunk
